...
/**
 * The base class for siteTransfer.
 */
class siteTransfer {
	/* @var modX $modx */
	public $modx;
	...
	function __construct(modX &$modx, array $config = array()) {
		...
		$corePath = $this->modx->getOption('sitetransfer_core_path', $config, $this->modx->getOption('core_path') . 'components/sitetransfer/');
		$assetsUrl = $this->modx->getOption('sitetransfer_assets_url', $config, $this->modx->getOption('assets_url') . 'components/sitetransfer/');
		...
		$this->modx->addPackage('sitetransfer', $this->config['modelPath']);
		$this->modx->lexicon->load('sitetransfer:default');
	}
	...
	public function loadManagerFiles(modManagerController $controller, modResource $resource) {
		...
		$properties = $resource->getProperties('sitetransfer');

		$controller->addLexiconTopic('sitetransfer:default');
		$controller->addJavascript($jsUrl . 'sitetransfer.js');
		...
		<script type="text/javascript">
			MODx.modx23 = ' . $modx23 . ';
			siteTransfer.config = ' . $this->modx->toJSON($this->config) . ';
			siteTransfer.config.resID = ' . $resource->id . ';
            siteTransfer.config.connector_url = "' . $this->config['connectorUrl'] . '";
		</script>', true);
